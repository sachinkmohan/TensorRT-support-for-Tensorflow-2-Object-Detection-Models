...
@gs.Graph.register()
def elt_const_clip(self, op, name, input, clip_min, clip_max):
    """
    ...
    """
    input_tensor = input if type(input) is gs.Variable else input[0]
    log.debug("Created {} node '{}".format(op, name))
    const_min = gs.Constant(name="{}_value:0".format(name), values=clip_min)
    const_max = gs.Constant(name="{}_value:1".format(name), values=clip_max)
    return self.layer(name=name, op=op, inputs=[input_tensor, const_min, const_max], outputs=[name + ":0"])

@gs.Graph.register()
def elt_const_slice(self, op, name, input, starts, ends, axes):
    """
    ...
    input_tensor = input if type(input) is gs.Variable else input[0]
    log.debug("Created {} node '{}".format(op, name))
    const_start = gs.Constant(name="{}_value:0".format(name), values=starts)
    const_end = gs.Constant(name="{}_value:1".format(name), values=ends)
    const_axes = gs.Constant(name="{}_value:2".format(name), values=axes)
    return self.layer(name=name, op=op, inputs=[input_tensor, const_start, const_end, const_axes], outputs=[name + ":0"])
